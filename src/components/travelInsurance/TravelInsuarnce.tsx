...
const TravelInsuarnce: React.FC = () => {
  ...
  const [totalInsurancePrice, setTotalInsurancePrice] = useState<number>(0);
  ...
  const geytInsurancePrice = () => {
    if (length > 0) {
      setfinishToogle(true);
      setRedAmount(true);
      setTotalInsurancePrice(
        calcPriceInsurance(country, typeOfTrip, health, length)
      );
    } else {
      setRedAmount(false);
    }
      ...
      {finishToogle ? (
        <div className={Styles.finalStep}>
          ...
          <div style={{ marginBottom: "2vh" }}>
            <p>Lebgth of stay: {length} Days</p>
            Total price for insurrance: {totalInsurancePrice}.00 kr
          </div>
          <div className={Styles.btnDiv}>
            <Button
              onClick={() => setfinishToogle(false)}
              id='uiBtn'
              variant='contained'
            >
              BACK
            ...
            <div className={Styles.container}>
              <form className={Styles.form} action=''>
                ...
                <input
                  ...
                  className={redAmount ? Styles.input : Styles.inputRed}
                  type='number'
                />
                {redAmount ? null : <p>PLease fill in</p>}
              </form>
              <div className={Styles.btnDiv}>
                <Button
